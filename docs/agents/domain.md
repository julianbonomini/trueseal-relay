...
- Also read **`trueseal-sync/CONTEXT.md`** for protocol-level terms (Envelope, Blob, Device, Keypair, Pairing) that trueseal-relay implements but does not own
...
When your output names a domain concept, use the term as defined in `CONTEXT.md`. For protocol-level terms not defined here, defer to `trueseal-sync/CONTEXT.md`.
